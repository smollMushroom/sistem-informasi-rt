...
const ProfileCard: FC<Props> = ({image, role, fullName, className}) => {
  return (
    <div className={`h-[341px] w-[273px] p-4 ${className} bg-secondary rounded-xl flex flex-col gap-2 justify-center items-center transition hover:scale-105`}>
      <img 
        src={image}
        alt={`photo-${fullName.split(' ').join('-')}`} 
        className="h-[204px] w-[163px]"  
      />
